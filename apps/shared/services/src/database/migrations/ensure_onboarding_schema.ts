/**
 * Migration: Ensure the onboarding + agent-assignment tables exist.
 *
 ...
 * 0000..N is impossible. The .sql files also never reach the runtime image (tsc
 * emits only .js into dist/), which means the idempotent 0013/0014 companions
 * have no execution path in a deployed container.
 *
 * So the DDL lives here, in TypeScript, mirroring EnsureSystemActor: compiled
 * into dist, invoked at boot, idempotent, safe to re-run. Without it the boot
 * backfill and every onboarding query hit tables that do not exist.
 *
 * Control plane only — user_agent_assignments deliberately has no FK to
 * agents (intelligence plane, separate pool); CrossPlaneGuard covers that.
 *
 * Standalone: bun apps/shared/services/src/database/migrations/ensure_onboarding_schema.ts
 * Programmatic: await new EnsureOnboardingSchema().run();
 */
...
const logger = createLogger({
  serviceName: 'migration:ensure-onboarding-schema',
  environment: process.env.NODE_ENV || 'development',
  logLevel: process.env.LOG_LEVEL || 'info',
});

/**
 * Ordering is load-bearing: tables, then the composite UNIQUE, then the FKs
 * that target it, then indexes. Postgres accepts a unique constraint as an FK
 * target but it must already exist — drizzle-kit emits them the other way
 * round, which fails on a real server (memory #3132).
 *
 * Every pg_constraint lookup compares against left(<name>, 63) because Postgres
 * truncates identifiers to 63 chars; comparing the full name never matches and
 * the ALTER runs twice.
 */
export const ONBOARDING_SCHEMA_STATEMENTS: readonly string[] = [
  ...
  `CREATE UNIQUE INDEX IF NOT EXISTS "uq_user_agent_assignments_user_agent" ON "user_agent_assignments" USING btree ("user_id","agent_id")`,
  `CREATE INDEX IF NOT EXISTS "idx_user_agent_assignments_user" ON "user_agent_assignments" USING btree ("user_id")`,
  `CREATE INDEX IF NOT EXISTS "idx_user_agent_assignments_org" ON "user_agent_assignments" USING btree ("organization_id")`,

  `CREATE UNIQUE INDEX IF NOT EXISTS "uq_onboarding_interviews_user_guide" ON "onboarding_interviews" USING btree ("organization_id","user_id","guide_agent_id")`,
  `CREATE INDEX IF NOT EXISTS "idx_onboarding_interviews_dropoff" ON "onboarding_interviews" USING btree ("organization_id","status","current_objective","last_activity_at") WHERE status IN ('active','paused')`,

  `CREATE UNIQUE INDEX IF NOT EXISTS "uq_onboarding_slot_evidence_chat" ON "onboarding_slot_evidence" USING btree ("interview_id","slot_key","slot_revision","source_message_id") WHERE source_kind = 'chat_message'`,
  ...
  `CREATE UNIQUE INDEX IF NOT EXISTS "uq_onboarding_extraction_runs_attempt" ON "onboarding_extraction_runs" USING btree ("interview_id","trigger_message_id","attempt_no")`,
  `CREATE UNIQUE INDEX IF NOT EXISTS "uq_onboarding_extraction_runs_accepted" ON "onboarding_extraction_runs" USING btree ("interview_id","trigger_message_id") WHERE outcome = 'accepted'`,
  `CREATE UNIQUE INDEX IF NOT EXISTS "uq_onboarding_extraction_runs_client_turn" ON "onboarding_extraction_runs" USING btree ("interview_id","client_turn_id") WHERE outcome = 'accepted'`,
];

/**
 * Arbitrary but FIXED key. Every booting machine must derive the same number
 * or the lock serializes nothing.
 */
export const ONBOARDING_SCHEMA_LOCK_KEY = 8_531_207;

export class EnsureOnboardingSchema {
  /**
   * `IF NOT EXISTS` and the `DO $$ IF NOT EXISTS (SELECT 1 FROM pg_constraint)`
   * blocks are check-then-act: two Fly machines booting together can both pass
   * the check and both run the ALTER. A session-level advisory lock serializes
   * them, and one transaction makes the 23 statements all-or-nothing, so a
   * failure can never leave the service running against half a schema.
   */
  async run(): Promise<void> {
    const client = await getControlPool().connect();

    try {
      await client.query('SELECT pg_advisory_lock($1)', [ONBOARDING_SCHEMA_LOCK_KEY]);
      ...
      logger.info('Onboarding schema ensured', {
        statements: ONBOARDING_SCHEMA_STATEMENTS.length,
      });
    } finally {
      await client.query('SELECT pg_advisory_unlock($1)', [ONBOARDING_SCHEMA_LOCK_KEY]);
      client.release();
    }
  }
}
