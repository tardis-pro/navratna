...
export class LLMService {
  ...
  // Context analysis
  async analyzeContext(request: ContextRequest): Promise<Record<string, unknown>> {
    try {
  ...
  private getPreferredProviderType(agent: Record<string, unknown>): string | undefined {
    ...
    const cfgEmpty: Record<string, unknown> = {};
    const cfgRecord = typeof cfg === 'object' && cfg !== null ? Object.assign(cfgEmpty, cfg) : null;
    const modelId =
  ...
  ): { cleanContent: string; toolCalls: LLMToolCall[] } {
