...
import { Eye, EyeOff, Lock, User, Lightbulb, Sparkles, Globe, MapPin, Github } from 'lucide-react';
...
// OAuth-only signup: new users sign up exclusively via GitHub or Google. Redirect the
// browser to the backend initiate endpoint, which 302s to the provider's consent screen.
      ...
      <div className="relative z-10 min-h-screen bg-gradient-to-br from-transparent via-slate-900/20 to-slate-900/40 flex items-center justify-center p-4">
        <div className="max-w-4xl w-full space-y-8">
          ...
          <div className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl rounded-2xl p-8 border border-slate-200/60 dark:border-slate-700/60 shadow-2xl shadow-slate-200/20 dark:shadow-slate-900/20">
            {/* OAuth signup — primary path for new users */}
            <div className="space-y-3 mb-6">
              <button
                type="button"
                onClick={() => startOAuth('github')}
                className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100 font-medium hover:bg-slate-50 dark:hover:bg-slate-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200 shadow-sm"
              >
