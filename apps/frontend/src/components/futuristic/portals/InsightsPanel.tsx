...
export const InsightsPanel: React.FC<InsightsPanelPortalProps> = ({ className, viewport }) => {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'anomaly':
        return <XMarkIcon className="w-6 h-6 text-purple-500" />;
      default:
        return <LightBulbIcon className="w-6 h-6 text-purple-500" />;
    }
  };
