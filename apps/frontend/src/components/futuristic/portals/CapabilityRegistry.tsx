...
export const CapabilityRegistry: React.FC<CapabilityRegistryPortalProps> = ({
  className,
  viewport,
}) => {
  ...
  const filteredCapabilities = capabilities.data.filter((cap) => {
    ...
    const matchesCategory =
      selectedCategory === 'all' || (cap.category || 'Uncategorized') === selectedCategory;
              ...
              {filteredCapabilities.map((capability) => (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span
                      className={`px-2 py-1 rounded-md text-xs font-medium border ${getStatusColor(capability.status)}`}
                    >
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white dark:bg-slate-700 rounded-xl p-4 border border-slate-200 dark:border-slate-600">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h4 className="font-semibold text-gray-900 dark:text-white">
                      {selectedCapabilityData.name}
                    </h4>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      ID: {selectedCapabilityData.id} • Category:{' '}
                      {selectedCapabilityData.category || 'Uncategorized'}
                    </p>
                  </div>
                  <span
                    className={`px-2 py-1 rounded-md text-xs font-medium border ${getStatusColor(selectedCapabilityData.status)}`}
                  >
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-medium text-gray-900 dark:text-white">
                      {selectedCapabilityData.successRate
                        ? `${(selectedCapabilityData.successRate * 100).toFixed(1)}%`
                        : 'N/A'}
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="font-medium text-gray-900 dark:text-white">
                      {selectedCapabilityData.avgDuration
                        ? `${selectedCapabilityData.avgDuration}ms`
                        : 'N/A'}
                    </p>
                  </div>
                  <div>
