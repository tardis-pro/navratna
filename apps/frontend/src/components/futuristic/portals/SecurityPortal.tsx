...
import { getStatusColor as getStatusToken, getSeverityColor as getSeverityToken } from '@/lib/status_tokens';
...
const SecurityPortalContent: React.FC<{
  ...
  const getStatusColor = (status: string) => getStatusToken(status, 'text');

  const getStatusBgColor = (status: string) => getStatusToken(status, 'bg');

  const getSeverityColor = (severity: string) => getSeverityToken(severity, 'soft');
