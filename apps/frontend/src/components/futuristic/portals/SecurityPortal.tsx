...
const SECURITY_METRICS: SecurityMetric[] = [
  { id: 'threats_blocked', label: 'Threats Blocked', value: 247, unit: 'today', trend: 'down', status: 'good', icon: ShieldCheck },
  { id: 'active_sessions', label: 'Active Sessions', value: 42, unit: 'current', trend: 'stable', status: 'good', icon: Users },
  { id: 'failed_logins', label: 'Failed Logins', value: 12, unit: 'last hour', trend: 'up', status: 'warning', icon: Lock },
  { id: 'system_uptime', label: 'System Uptime', value: 99.97, unit: '%', trend: 'stable', status: 'good', icon: Server },
  { id: 'vulnerabilities', label: 'Open Vulnerabilities', value: 3, unit: 'total', trend: 'down', status: 'warning', icon: Bug },
  { id: 'data_encrypted', label: 'Data Encrypted', value: 100, unit: '%', trend: 'stable', status: 'good', icon: Key }
];

const SECURITY_EVENTS: SecurityEvent[] = [
  { id: '1', type: 'threat', severity: 'high', message: 'Suspicious login attempt blocked from IP 192.168.1.100', timestamp: new Date(Date.now() - 5 * 60000), source: 'Authentication Service', resolved: true },
  { id: '2', type: 'access', severity: 'medium', message: 'Admin user john.doe accessed sensitive data', timestamp: new Date(Date.now() - 15 * 60000), source: 'Audit Trail', resolved: false },
  { id: '3', type: 'system', severity: 'low', message: 'Security patch applied to database server', timestamp: new Date(Date.now() - 30 * 60000), source: 'System Updates', resolved: true },
  { id: '4', type: 'threat', severity: 'critical', message: 'Potential DDoS attack detected and mitigated', timestamp: new Date(Date.now() - 45 * 60000), source: 'Network Monitor', resolved: true },
...
const SecurityPortalContent: React.FC<{
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const statuses: ('healthy' | 'warning' | 'critical')[] = ['healthy', 'healthy', 'healthy', 'warning', 'healthy'];
    ...
    <div className="h-full bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white overflow-hidden">
      ...
      >
        <div className={`${isCompactMode ? 'px-3 py-3' : 'px-6 py-6'}`}>
          <div className={`flex items-center justify-between ${isCompactMode ? 'flex-col gap-3' : 'flex-wrap gap-4'}`}>
            <div className={`flex items-center ${isCompactMode ? 'flex-col text-center gap-2' : 'gap-4'}`}>
              <motion.div 
                ...
                animate={{ 
                  boxShadow: systemStatus === 'critical' 
                    ? ['0 0 0 rgba(239, 68, 68, 0)', '0 0 20px rgba(239, 68, 68, 0.5)', '0 0 0 rgba(239, 68, 68, 0)']
                    : systemStatus === 'warning'
                    ? ['0 0 0 rgba(245, 158, 11, 0)', '0 0 15px rgba(245, 158, 11, 0.4)', '0 0 0 rgba(245, 158, 11, 0)']
              ...
              <div>
                ...
                {!isCompactMode && (
                  <p className="text-slate-400">Real-time security monitoring and threat detection</p>
                )}
              </div>
            </div>
            ...
            <div className={`flex items-center ${isCompactMode ? 'gap-2' : 'gap-3'}`}>
              {!isVerySmall && (
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className={`${isCompactMode ? 'text-xs' : 'text-sm'} text-slate-300 capitalize`}>
              ...
              >
                {realTimeData ? <Radio className={`${isCompactMode ? 'w-3 h-3' : 'w-4 h-4'}`} /> : <Pause className={`${isCompactMode ? 'w-3 h-3' : 'w-4 h-4'}`} />}
              </button>
              ...
              <button
                ...
                title="Refresh data"
              >
                <RefreshCw className={`${isCompactMode ? 'w-3 h-3' : 'w-4 h-4'} ${loading ? 'animate-spin' : ''}`} />
              </button>
          ...
          {activeTab === 'overview' && (
            ...
            >
              ...
              <div className={`grid gap-4 ${
                ...
                {(showReducedMetrics ? SECURITY_METRICS.slice(0, 4) : SECURITY_METRICS).map((metric, index) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`flex items-center justify-between ${isCompactMode ? 'mb-2' : 'mb-4'}`}>
                        <div className={`${isCompactMode ? 'w-8 h-8' : 'w-12 h-12'} rounded-xl flex items-center justify-center ${getStatusBgColor(metric.status)}`}>
                          <Icon className={`${isCompactMode ? 'w-4 h-4' : 'w-6 h-6'} ${getStatusColor(metric.status)}`} />
                        </div>
                        <div className={`flex items-center gap-1 text-xs ${getStatusColor(metric.trend === 'up' ? (metric.status === 'good' ? 'good' : 'warning') : 'good')}`}>
                          {getTrendIcon(metric.trend)}
                          {!isCompactMode && <span className="capitalize">{metric.trend}</span>}
                        </div>
                      </div>
                      <div className="space-y-1">
                        <p className={`${isCompactMode ? 'text-lg' : 'text-2xl'} font-bold text-white`}>
              ...
              {!isCompactMode && (
                ...
                >
                  <div className={`flex items-center justify-between ${isCompactMode ? 'mb-3' : 'mb-6'}`}>
                    <h3 className={`${isCompactMode ? 'text-base' : 'text-lg'} font-semibold text-white flex items-center gap-2`}>
                      <Bell className={`${isCompactMode ? 'w-4 h-4' : 'w-5 h-5'} text-orange-400`} />
                  ...
                  <div className="space-y-3">
                    {SECURITY_EVENTS.slice(0, isCompactMode ? 3 : 5).map((event, index) => (
                      ...
                      >
                        <div className={`${isCompactMode ? 'w-6 h-6' : 'w-8 h-8'} rounded-lg flex items-center justify-center flex-shrink-0 ${getSeverityColor(event.severity)}`}>
                          {event.type === 'threat' && <ShieldAlert className={`${isCompactMode ? 'w-3 h-3' : 'w-4 h-4'}`} />}
                          {event.type === 'access' && <Key className={`${isCompactMode ? 'w-3 h-3' : 'w-4 h-4'}`} />}
                          {event.type === 'system' && <Server className={`${isCompactMode ? 'w-3 h-3' : 'w-4 h-4'}`} />}
                          {event.type === 'audit' && <FileText className={`${isCompactMode ? 'w-3 h-3' : 'w-4 h-4'}`} />}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className={`${isCompactMode ? 'text-xs' : 'text-sm'} text-white font-medium mb-1`}>
                            {isCompactMode ? event.message.slice(0, 50) + '...' : event.message}
                          </p>
                          <div className={`flex items-center gap-2 ${isCompactMode ? 'text-xs' : 'text-xs'} text-slate-400`}>
                            {!isVerySmall && (
                              <>
                                <span>{isCompactMode ? event.source.split(' ')[0] : event.source}</span>
                                <span>•</span>
                              </>
                            )}
                            <span>{formatTimestamp(event.timestamp)}</span>
                            <span>•</span>
                            <span className={`px-2 py-1 rounded-full ${getSeverityColor(event.severity)} font-medium`}>
                              {isCompactMode ? event.severity.slice(0, 1).toUpperCase() : event.severity.toUpperCase()}
                            </span>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          {event.resolved ? (
                            <CheckCircle className={`${isCompactMode ? 'w-3 h-3' : 'w-4 h-4'} text-green-400`} />
                          ) : (
                            <AlertTriangle className={`${isCompactMode ? 'w-3 h-3' : 'w-4 h-4'} text-yellow-400`} />
                          )}
                        </div>
                      </motion.div>
                    ))}
          ...
          {activeTab === 'users' && (
            ...
            >
              <div className="text-center py-12">
                ...
                <p className="text-slate-400">Manage user access, permissions, and security policies</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {!isVerySmall && (
        ...
        >
          <div className={`flex items-center gap-2 ${isCompactMode ? 'px-3 py-2' : 'px-4 py-3'} bg-slate-900/90 backdrop-blur-xl border border-slate-700/50 rounded-xl shadow-2xl`}>
            <div className="flex items-center gap-2">
              <motion.div 
                className={`w-3 h-3 rounded-full ${
                  systemStatus === 'healthy' ? 'bg-green-400' :
              ...
              />
              <span className={`${isCompactMode ? 'text-xs' : 'text-sm'} text-slate-300 font-medium`}>
