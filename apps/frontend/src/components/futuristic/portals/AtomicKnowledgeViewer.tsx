...
export const AtomicKnowledgeViewer: React.FC<AtomicKnowledgeViewerProps> = ({
  ...
  const analyzeSentiment = (content: string): 'positive' | 'neutral' | 'negative' => {
    ...
    const positiveWords = ['good', 'great', 'excellent', 'amazing', 'wonderful', 'fantastic', 'positive', 'success', 'achieve'];
  ...
  const getComplexityIcon = (complexity: string) => {
    switch (complexity) {
      case 'simple': return <CheckCircle2 className="w-4 h-4 text-green-400" />;
      case 'complex': return <AlertCircle className="w-4 h-4 text-red-400" />;
  ...
  const getConnectionTypeColor = (type: string) => {
    switch (type) {
      ...
      case 'temporal': return 'bg-green-500/20 text-green-400 border-green-500/30';
      case 'causal': return 'bg-purple-500/20 text-purple-400 border-purple-500/30';
      case 'reference': return 'bg-orange-500/20 text-orange-400 border-orange-500/30';
      ...
      <div className="p-6 border-b border-slate-700/50 bg-black/20 backdrop-blur-sm">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <DiscussionTrigger
              trigger={
                <Button size="sm" variant="outline" className="bg-blue-600/20 hover:bg-blue-600/30 border-blue-500/50">
            ...
            {relatedItems.length > 0 ? (
              <ScrollArea className="h-full">
                <div className="space-y-3 pr-4">
                  {relatedItems.map((relatedItem) => (
                    <Card key={relatedItem.id} className="bg-black/20 border-slate-700/50 hover:bg-slate-800/30 transition-colors cursor-pointer">
              ...
              <div className="flex items-center justify-center h-full">
                <div className="text-center">
                  ...
                  <p className="text-slate-500 text-sm">Related knowledge will appear here as you build your knowledge base</p>
                </div>
              </div>
            )}
          </TabsContent>
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      {analysis.keyTerms.map((term, index) => (
                        <div key={term} className="flex items-center justify-between p-2 bg-slate-800/30 rounded">
