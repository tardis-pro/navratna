...
export const MapWallpaper: React.FC<MapWallpaperProps> = React.memo(({
  ...
  const initializeMap = useCallback(() => {
    ...
    const selectedBasemap = BASEMAP_OPTIONS.find(b => b.id === currentBasemap) || BASEMAP_OPTIONS[0];
          ...
          >
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg backdrop-blur-md 
              ...
              <span className="text-xs">
                {isInteractive ? 'Interactive' : 'Static'} • {BASEMAP_OPTIONS.find(b => b.id === currentBasemap)?.name}
              </span>
            </div>
          </motion.div>
        )}
