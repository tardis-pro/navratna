...
export const ThoughtProcess: React.FC<ThoughtProcessProps> = ({ messages, className }) => {
  ...
  const filteredThoughts = useMemo(() => {
    ...
    return thoughtSteps.filter((step) => step.content.toLowerCase().includes(query));
    ...
    >
      <div className="flex items-center justify-between mb-4">
        ...
        >
          ...
          <span className="font-medium">Thought Process ({thoughtSteps.length} steps)</span>
        </button>
        {isExpanded && (
          ...
          {filteredThoughts.map((step, index) => (
            ...
            >
              <div className="flex items-start gap-4">
                <div
                  className="flex-shrink-0 w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-900/30 
