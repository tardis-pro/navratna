...
export const DiscussionPortal: React.FC<DiscussionPortalProps> = ({
  ...
  const handleStartDiscussion = async () => {
    ...
    try {
      await start(topic, selectedAgents.length > 0 ? selectedAgents : undefined, discussionData);
  ...
  const renderMessage = (message: Message, index: number) => {
    const agent = Object.values(agents).find(
      (a) => a.name === message.sender || a.id === message.sender
    );
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <div className="flex items-center space-x-2">
                  ...
                  {agent && (
                    <span
                      className={`px-2 py-1 text-xs rounded-full border ${getAgentColorClasses(
                        color
                      )
                        .split(' ')
                        .slice(2)
    ...
    <div className="flex items-center justify-between p-4 bg-slate-800/50 border-b border-slate-700/50">
      <div className="flex items-center gap-3">
        ...
        <div className="text-sm text-slate-300">
          {mode === 'discussion'
            ? 'Configure and manage AI discussions'
            : mode === 'monitor'
              ? 'Monitor discussion performance and status'
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-lg font-bold text-white">
                        {stat.isText ? (
                          <span
                            className={`text-sm font-semibold ${isActive ? 'text-green-400' : 'text-slate-400'}`}
                          >
                            {stat.value}
                          </span>
                        ) : (
                    ...
                    <div
                      className={`p-2 rounded-lg bg-gradient-to-br from-${stat.color}-500/20 to-${stat.color}-600/20 border border-${stat.color}-500/30`}
                    >
              ...
              <AnimatePresence>
                {!canStart && (
                  ...
                  >
                    ...
                    <div>
                      <span className="text-amber-300 font-medium block">Insufficient Agents</span>
