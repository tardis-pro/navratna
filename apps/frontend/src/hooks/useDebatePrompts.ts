  ...
  const constructLlamaPrompt = useMemo(() => {
    return (
      ...
      const phaseInstructions: Record<DebatePhase, string> = {
        opening:
          'Establish your key position and strongest arguments. Anticipate counterarguments.',
        rebuttal:
          "Directly address your opponent's strongest points. Steelman their argument first before refuting.",
        'cross-examination':
          "Probe inconsistencies and ask targeted questions that reveal weaknesses in your opponent's position.",
        conclusion:
          'Synthesize the debate, emphasize your strongest points, reiterate your core thesis, and address remaining objections persuasively.',
      };
      ...
      const generalRepetition =
        new Set(lastMessages.map((m) => m.content.slice(0, 30))).size <
        Math.max(1, lastMessages.length * 0.75);
  ...
  const constructJudgePrompt = useMemo(() => {
    return (options?: { includeContext?: boolean }) => {
      ...
      const possibleLoop =
        ...
        new Set(lastMessages.map((m) => m.content.substring(0, 40))).size <
          Math.max(1, lastMessages.length * 0.8);
