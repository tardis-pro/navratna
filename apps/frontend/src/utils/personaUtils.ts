import { Persona, PersonaTrait, PersonaTone, PersonaStyle } from '../types/persona';
import { 
  HybridPersona, 
  ConversationContext, 
  MessageHistoryItem, 
...
Leverage your dual expertise naturally, don't force it. /no_think`;
  ...
  return {
    ...
    dominantExpertise: dominantPersona.expertise[0],
    personalityBlend,
    // Blend conversational characteristics
    tone: dominantPersona.tone,
    style: secondaryPersona.style, // Mix styles for interesting hybrids
    energyLevel: dominantPersona.energyLevel,
    chattiness: (dominantPersona.chattiness + secondaryPersona.chattiness) / 2,
    empathyLevel: Math.max(dominantPersona.empathyLevel, secondaryPersona.empathyLevel)
  };
}
...
export function calculateContributionScore(
  ...
  const momentumMatch = (context.conversationMomentum === 'clarifying' && persona.id === 'junior-developer') ? 0.5 : 0.2;
  
  // Factor 3: Chattiness factor (persona's natural tendency to contribute)
  const chattinessFactor = persona.chattiness || 0.5;
  ...
  const energyBonus = (persona.energyLevel === 'high' || persona.energyLevel === 'dynamic') 
    ? conversationState.conversationEnergy * 0.2 : 0.0;
  
  const totalScore = topicMatch + momentumMatch + chattinessFactor + continuityPenalty + energyBonus;
...
export function shouldPersonaContribute(
  ...
  const score = calculateContributionScore(persona, context, currentTopic, contextualTriggers, conversationState);
...
export function getResponseStarter(
  ...
  const { tone, style } = persona;
  
  const starters: Record<PersonaTone, Record<ResponseType, string[]>> = {
    casual: {
    ...
    analytical: {
      primary: ['Looking at this from...', 'I think we need to consider...', 'The data suggests...'],
    ...
    empathetic: {
      primary: ['I understand that...', 'From a human perspective...', 'Considering how this affects people...'],
    ...
    verbose: {
      primary: ['I\'d like to elaborate on this topic because...', 'Let me walk through my thinking here...'],
      'follow-up': ['I want to expand on that excellent point and add some additional considerations...'],
    ...
    humorous: {
      primary: ['Well, this is interesting! 😄', 'OK, so here\'s my take...', 'Alright, buckle up...'],
    ...
    cautious: {
      primary: ['I want to be careful here, but...', 'Let me think through this...', 'We should proceed carefully...'],
      'follow-up': ['I also want to mention, with some caution...', 'Additionally, though I\'m hesitant to say...'],
      ...
      clarification: ['Before we proceed, could you clarify...', 'I want to make sure I understand...']
    },
    optimistic: {
      primary: ['This is exciting!', 'I love where this is going!', 'What a great opportunity to...'],
      'follow-up': ['And here\'s another exciting aspect...', 'This gets even better...'],
      agreement: ['Absolutely! That\'s fantastic!', 'Yes! I\'m totally on board!'],
      concern: ['I\'m optimistic we can work through...', 'This is just a small hurdle...'],
      transition: ['Let\'s explore this amazing opportunity...', 'Moving to an even more exciting topic...'],
      clarification: ['I\'d love to understand more about...', 'Could you share more about this exciting...']
    }
  };
  
  const toneStarters = starters[tone]?.[responseType] || starters.casual[responseType];
...
export function getFiller(persona: Persona, fillerType: FillerType): string {
  const fillers: Record<FillerType, Record<PersonaTone, string[]>> = {
    thinking: {
      casual: ['Let me think...', 'Hmm...', 'So...'],
      analytical: ['Let me analyze this...', 'Processing this...', 'Evaluating...'],
      empathetic: ['I\'m reflecting on this...', 'Let me consider...'],
      concise: ['...', 'Thinking.', 'Hmm.'],
      verbose: ['I\'m taking a moment to carefully consider the implications...'],
      humorous: ['*thinking face* 🤔', 'Give me a sec... *brain loading*'],
      cautious: ['Let me carefully think this through...', 'I need to consider this carefully...'],
      optimistic: ['Ooh, let me think about the possibilities...', 'This is interesting, let me think...']
    },
    hesitation: {
      casual: ['Well...', 'I mean...', 'Like...'],
      analytical: ['However...', 'That said...', 'On the other hand...'],
      empathetic: ['I feel like...', 'It seems to me...'],
      concise: ['But...', 'Though...'],
      verbose: ['I find myself somewhat hesitant to say this, but...'],
      humorous: ['Uh... well... 😅', '*nervous laugh*'],
      cautious: ['I\'m hesitant to say this, but...', 'I\'m not entirely sure, but...'],
      optimistic: ['Well, I hope...', 'I\'m thinking maybe...']
    },
    transition: {
      casual: ['Anyway...', 'So...', 'But yeah...'],
      analytical: ['Therefore...', 'Consequently...', 'As a result...'],
      empathetic: ['I also feel...', 'What\'s important to me is...'],
      concise: ['Next:', 'Also:'],
      verbose: ['Moving forward with this line of thinking...'],
      humorous: ['*drum roll*', 'And now...', 'Plot twist!'],
      cautious: ['Moving carefully to...', 'Let me cautiously transition to...'],
      optimistic: ['And excitingly...', 'Even better...']
    },
    agreement: {
      casual: ['Yeah!', 'Totally!', 'For sure!'],
      analytical: ['Precisely.', 'Correct.', 'That\'s accurate.'],
      empathetic: ['I feel that too.', 'That resonates.'],
      concise: ['Yes.', 'Agreed.'],
      verbose: ['I find myself in complete agreement with that sentiment...'],
      humorous: ['Boom! 💥', 'Nailed it!', 'Ding ding! 🔔'],
      cautious: ['I believe so...', 'That seems right...'],
      optimistic: ['Absolutely!', 'Love it!', 'Perfect!']
    },
    casual: {
      casual: ['You know...', 'I guess...', 'Maybe...'],
      analytical: ['In fact...', 'Indeed...', 'Actually...'],
      empathetic: ['I sense...', 'It feels like...'],
      concise: ['Well...'],
      verbose: ['As I was contemplating this...'],
      humorous: ['LOL', '*shrugs*', 'Haha'],
      cautious: ['Perhaps...', 'Possibly...'],
      optimistic: ['Awesome!', 'Great!', 'Cool!']
    }
  };
  
  const personaFillers = fillers[fillerType]?.[persona.tone] || fillers[fillerType].casual;
...
export function getEmotionalReflection(
  ...
  if (persona.empathyLevel < 0.6) return ''; // Only high-empathy personas reflect
...
export function generateEnhancedResponse(
  ...
  if (responseEnhancement.addEmotionalReflection && persona.empathyLevel > 0.6) {
  ...
  if (persona.style === 'inquisitive' && !response.includes('?')) {
    const questions = ['What do you all think?', 'Does that make sense?', 'Am I missing something?'];
...
export function updateConversationState(
  ...
  return {
    ...
    lastSpeakerContinuityCount: state.activePersonaId === newSpeaker ? state.lastSpeakerContinuityCount + 1 : 1,
...
export function detectTopicShift(
  ...
  const currentWords = currentMessage.toLowerCase().split(' ').filter(word => word.length > 3);
  const previousWords = previousTopics.join(' ').toLowerCase().split(' ').filter(word => word.length > 3);
...
export function createConversationContext(
  ...
  // Detect topic shifts
  const topicShiftDetected = messageHistory.length > 1 
...
function analyzeOverallTone(recentMessages: string[]): 'formal' | 'casual' | 'heated' | 'collaborative' {
  ...
  if (allText.includes('therefore') || allText.includes('furthermore') || allText.includes('however')) {
    return 'formal';
  }
...
function analyzeEmotionalContext(lastMessage: string): 'neutral' | 'excited' | 'concerned' | 'frustrated' | 'optimistic' {
