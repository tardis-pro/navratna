  ...
  const recordAnswerMutation = useMutation({
    ...
    onSuccess: (result: InterviewAnswer) => {
      ...
      if (result.resolvedAssumptions.length > 0) {
        toast.success(
          `${result.resolvedAssumptions.length} assumption(s) resolved`
        );
      }
      if (result.newContradictions.length > 0) {
        toast.warning(
          `${result.newContradictions.length} new contradiction(s) detected`
        );
      }
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-3">
                <Button
                  ...
                  disabled={
                    !currentAnswer.trim() || isPaused || isRecording
                  }
                >
                  {isRecording ? (
                    <>Recording...</>
                ...
                {lastAnswer.followUpNeeded && (
                  <div className="flex items-center gap-2 text-amber-600 bg-amber-50 p-3 rounded-md">
                    ...
                    <span className="text-sm font-medium">
                      Follow-up needed for this answer
                    </span>
                  </div>
                )}
            ...
            {questions.map((q, idx) => {
              ...
              return (
                <button
                  key={q.id}
                  onClick={() =>
                    setExpandedAnswerIndex(isExpanded ? null : idx)
                  }
