describe('DiscussionOrchestrationService', () => {
  ...
  beforeEach(() => {
    ...
    mockDiscussionOrchestrationService.createDiscussion.mockResolvedValue({
      success: true,
      data: {
        id: 'discussion-123',
    ...
    mockDiscussionOrchestrationService.startDiscussion.mockResolvedValue({
      success: true,
      data: {
        id: 'discussion-123',
        status: 'active',
    ...
    mockDiscussionOrchestrationService.addParticipant.mockResolvedValue({
      success: true,
      data: {
        id: 'participant-123',
        agentId: 'agent-123',
    ...
    mockDiscussionOrchestrationService.sendMessage.mockResolvedValue({
      success: true,
      data: {
        id: 'message-123',
    ...
    mockDiscussionOrchestrationService.advanceTurn.mockResolvedValue({
      success: true,
      data: {
    ...
    mockDiscussionOrchestrationService.pauseDiscussion.mockResolvedValue({
      success: true,
      data: { id: 'discussion-123', status: 'paused' },
      events: [{ type: 'status_changed', data: { newStatus: 'paused' } }],
    });

    mockDiscussionOrchestrationService.resumeDiscussion.mockResolvedValue({
      success: true,
      data: { id: 'discussion-123', status: 'active' },
      events: [{ type: 'status_changed', data: { newStatus: 'active' } }],
    });

    mockDiscussionOrchestrationService.endDiscussion.mockResolvedValue({
      success: true,
      data: { id: 'discussion-123', status: 'completed' },
      events: [{ type: 'status_changed', data: { newStatus: 'completed' } }],
    });

    mockDiscussionOrchestrationService.getDiscussion.mockResolvedValue({
      id: 'discussion-123',
      title: 'Test Discussion',
      status: 'active',
      participants: [
    ...
    mockDiscussionOrchestrationService.verifyParticipantAccess.mockResolvedValue(true);

    mockDiscussionOrchestrationService.getParticipantByUserId.mockResolvedValue({
      id: 'participant-123',
      userId: 'user-123',
      agentId: 'agent-123',
      isActive: true,
    ...
    mockDiscussionOrchestrationService.requestTurn.mockResolvedValue({
      success: true,
      data: { status: 'active', message: 'It is already your turn' },
    });

    mockDiscussionOrchestrationService.endTurn.mockResolvedValue({
      success: true,
      data: { message: 'Turn ended successfully', nextParticipant: { id: 'participant-2' } },
    });

    mockDiscussionOrchestrationService.addReaction.mockResolvedValue({
      success: true,
      data: {
        id: 'reaction-123',
        participantId: 'participant-123',
    ...
    mockDiscussionOrchestrationService.getStatus.mockReturnValue({
      activeDiscussions: 2,
      activeTurnTimers: 1,
      cacheSize: 2,
      uptime: 3600,
    ...
    mockDiscussionOrchestrationService.cleanup.mockResolvedValue(undefined);
  ...
  describe('getDiscussion', () => {
    ...
    it('should handle discussion not found', async () => {
      mockDiscussionOrchestrationService.getDiscussion.mockResolvedValue(null);
  ...
  describe('verifyParticipantAccess', () => {
    ...
    it('should deny access for invalid participant', async () => {
      mockDiscussionOrchestrationService.verifyParticipantAccess.mockResolvedValue(false);
  ...
  describe('getParticipantByUserId', () => {
    ...
    it('should handle participant not found', async () => {
      mockDiscussionOrchestrationService.getParticipantByUserId.mockResolvedValue(null);
