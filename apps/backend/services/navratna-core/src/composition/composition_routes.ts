  ...
  return new Elysia()
    ...
    .get('/api/v1/compositions/:id/instance-state', async ({ params, set, headers, request }) => {
      ...
      try {
        ...
        if (!instance) {
          set.status = 404
          return { success: false, error: 'No instance state found for this composition' }
        }

        const payload: WorkflowInstanceStatePayload = {
        ...
        logger.error('composition: get instance-state failed', {
...
export async function publishWorkflowStateChanged(
  ...
  payload: WorkflowInstanceStatePayload,
): Promise<void> {
  try {
    const bus = EventBusService.getInstance()
    await bus.publish('workflow.state-changed', {
      compositionId,
      ...payload,
    })
  } catch (error) {
    logger.error('composition: failed to publish workflow.state-changed', {
      compositionId,
      error: error instanceof Error ? error.message : String(error),
    })
  }
