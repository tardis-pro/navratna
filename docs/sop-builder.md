...
environment: - DATABASE_URL=${DATABASE_URL}
      - RABBITMQ_URL=${RABBITMQ_URL} - NEO4J_URL=${NEO4J_URL}
depends_on: - postgres - rabbitmq - neo4j
...
export class BusinessProcessStrategy extends ContextAwareStrategy {
  ...
  private calculateProcessScore(agent: Agent, step: BusinessProcessStep): number {
    ...
    const toolsMatch = step.requiredTools.every((tool) => agent.capabilities.includes(tool))
      ? 0.3
      : 0;
