...
export class AgentLLMPreferenceRepository {
  constructor(private repository: Repository<AgentLLMPreference>) {}
  ...
  async findByAgentAndTask(
    agentId: string,
    taskType: LLMTaskType
  ): Promise<AgentLLMPreference | null> {
  ...
  async findOptimalAgentsForTask(
    taskType: LLMTaskType,
    limit: number = 10
  ): Promise<AgentLLMPreference[]> {
  ...
  async getAgentPreferenceStats(agentId: string): Promise<{
    ...
    const qualityScores = preferences
  ...
  async getTaskAnalysis(taskType: LLMTaskType): Promise<{
    ...
    const modelCounts = preferences.reduce(
      (acc, pref) => {
    ...
    const providerCounts = preferences.reduce(
      (acc, pref) => {
    ...
    const settings = highPerformers
      .map((p) => p.getEffectiveSettings())
      .filter((s) => s.temperature !== undefined);

    const recommendedSettings = {
      temperature:
        settings.length > 0
          ? settings.reduce((sum, s) => sum + (s.temperature || 0), 0) / settings.length
          : undefined,
      maxTokens:
        settings.length > 0
          ? Math.round(settings.reduce((sum, s) => sum + (s.maxTokens || 0), 0) / settings.length)
          : undefined,
    };
