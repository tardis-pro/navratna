  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!user || !user.userPersona) {
        res
          .status(400)
          .json({ error: 'User persona not found. Please complete onboarding first.' });
        return;
      }
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const { resetPersona, resetProviders, resetModels, detectCapabilities, reason } =
        validation.data;
      const databaseService = DatabaseService.getInstance();
      const userRepository = databaseService.getUserRepository();
      const user = await userRepository.findById(userId);
      ...
      if (resetModels) {
        try {
          ...
          await DefaultUserLLMProviderSeed.updateDefaultProviders(
            await databaseService.getDataSource()
          );
...
async function getCompatibleAgents(persona: any) {
  ...
  const compatibleAgents = allAgents.filter((agent) => {
    ...
    if (
      persona.communicationPreference === 'brief' &&
      agent.configuration?.responseStyle === 'detailed'
    ) {
      return false;
    }
...
async function generateOptimizedWorkspace(persona: any, behavioralPatterns: any) {
  ...
  if (persona.problemSolvingApproach === 'analytical') {
    workspace.recommendations.widgets.push(
      'analytics-dashboard',
      'data-insights',
      'performance-metrics'
    );
  ...
  if (behavioralPatterns?.frequentlyUsedTools) {
    workspace.recommendations.shortcuts = behavioralPatterns.frequentlyUsedTools.map(
      (tool: string) => ({
        tool,
        priority: 'high',
        placement: 'sidebar',
  ...
  if (behavioralPatterns?.preferredAgents) {
    workspace.recommendations.agentPlacements = behavioralPatterns.preferredAgents.map(
      (agentId: string) => ({
        agentId,
...
function calculateCompatibilityScore(persona: any, agent: any): number {
  ...
  if (persona.communicationPreference && agent.configuration?.responseStyle) {
    ...
    if (persona.communicationPreference === agent.configuration.responseStyle) {
      ...
      persona.communicationPreference === 'detailed' &&
      agent.configuration.responseStyle === 'comprehensive'
    ) {
      score += 20;
    }
  }
