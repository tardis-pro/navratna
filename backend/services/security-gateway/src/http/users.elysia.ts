...
export function registerUserRoutes(app: any): any {
  return app.group('/api/v1/users', (app: any) => withOptionalAuth(app)
    ...
    .get('/public', async ({ set, query }) => {
      ...
      try {
        const { userService } = await getServices();
        const repo = userService.getUserRepository();
        const result = await repo.searchUsers({ search: search as any, role: 'user' as any, isActive: true as any, limit, offset });
        ...
        return {
          ...
          data: { users: publicUsers, pagination: { page, limit, total: result.total, pages: Math.ceil(result.total / limit) } }
        };
      } catch (error) {
        set.status = 500;
        return { success: false, error: 'Internal Server Error', message: 'Failed to retrieve public users' };
      }
    })
    ...
    // PUT /api/v1/users/llm-preferences
      .put('/llm-preferences', async ({ set, user, body }) => {
        ...
        try {
          const { userService } = await getServices();
          const repo = userService.getUserLLMPreferenceRepository();
        await repo.bulkUpsert(parsed.data.preferences.map((p) => ({ ...p, userId: user!.id })) as any);
    ...
    .group('', (g: any) => withAdminGuard(g)
      .get('/:userId', async ({ set, params }) => {
        try {
          ...
          if (!user) { set.status = 404; return { error: 'User Not Found', message: 'User not found' }; }
    ...
    // POST /api/v1/users (admin)
      .post('/', async ({ set, body, user }) => {
        ...
        try {
          ...
          if (existing) { set.status = 409; return { error: 'User Already Exists', message: 'A user with this email already exists' }; }
          ...
          userId: user!.id,
            details: { createdUserId: created.id, createdUserEmail: created.email, createdUserRole: created.role, isActive: created.isActive },
            ipAddress: '', userAgent: ''
          });
    ...
    // PUT /api/v1/users/:userId (admin)
      .put('/:userId', async ({ set, body, params }) => {
        ...
        try {
          ...
          if (!current) { set.status = 404; return { error: 'User Not Found', message: 'User not found' }; }
          if (parsed.data.email && parsed.data.email !== current.email) {
            const emailCheck = await userService.findUserByEmail(parsed.data.email);
            if (emailCheck && emailCheck.id !== params.userId) { set.status = 409; return { error: 'Email Already Exists', message: 'Another user with this email already exists' }; }
          }
          const repo = userService.getUserRepository();
          ...
          if (!updated) { set.status = 500; return { error: 'Update Failed', message: 'Failed to update user' }; }
          if (parsed.data.email && parsed.data.email !== current.email) {
          ...
          userId: undefined,
            details: { updatedUserId: updated.id, updatedUserEmail: updated.email, updatedFields: Object.keys(parsed.data), previousRole: (current as any).role, newRole: (updated as any).role, previousActive: (current as any).isActive, newActive: (updated as any).isActive },
            ipAddress: '', userAgent: ''
          });
    ...
    // DELETE /api/v1/users/:userId (admin)
      .delete('/:userId', async ({ set, params }) => {
        try {
          ...
          if (!ok) { set.status = 404; return { error: 'User Not Found', message: 'User not found' }; }
          await auditService.logSecurityEvent({
