...
export function registerPersonaRoutes(app: any): any {
  return app.group('/api/v1/user/persona', (app: any) => withRequiredAuth(app)
    // GET /
    .get('/', async ({ set, user }) => {
      try {
        ...
        if (!entity) { set.status = 404; return { error: 'User not found' }; }
        return { id: entity.id, email: entity.email, firstName: entity.firstName, lastName: entity.lastName, userPersona: entity.userPersona, onboardingProgress: entity.onboardingProgress, behavioralPatterns: entity.behavioralPatterns, updatedAt: entity.updatedAt };
      } catch (error) { set.status = 500; return { error: 'Internal server error' }; }
    })

    // PUT /
    .put('/', async ({ set, body, user }) => {
      const validation = UpdatePersonaSchema.safeParse(body);
      if (!validation.success) { set.status = 400; return { error: 'Invalid request data', details: validation.error.errors }; }
      try {
        const databaseService = DatabaseService.getInstance();
        const repo = databaseService.getUserRepository();
        const entity = await repo.findById(user!.id);
        ...
        if (onboardingProgress) entity.onboardingProgress = { ...entity.onboardingProgress, ...onboardingProgress } as any;
        if (behavioralPatterns) entity.behavioralPatterns = { ...entity.behavioralPatterns, ...behavioralPatterns } as any;
        await repo.update(user!.id, entity);
        logger.info('User persona updated', { userId: user!.id, updatedFields: Object.keys(body as any) });
        return { id: entity.id, email: entity.email, firstName: entity.firstName, lastName: entity.lastName, userPersona: entity.userPersona, onboardingProgress: entity.onboardingProgress, behavioralPatterns: entity.behavioralPatterns, updatedAt: entity.updatedAt };
      } catch (error) { set.status = 500; return { error: 'Internal server error' }; }
    })
    ...
    .post('/complete-onboarding', async ({ set, body, user }) => {
      ...
      if (!validation.success) { set.status = 400; return { error: 'Invalid onboarding data', details: validation.error.errors }; }
      try {
        const databaseService = DatabaseService.getInstance();
        const repo = databaseService.getUserRepository();
        const entity = await repo.findById(user!.id);
        ...
        entity.onboardingProgress = { ...onboardingProgress, isCompleted: true, completedAt: new Date(), currentStep: onboardingProgress.currentStep || 0, completedSteps: onboardingProgress.completedSteps || [], responses: onboardingProgress.responses || {} } as any;
        entity.behavioralPatterns = { sessionDuration: 0, activeHours: [], frequentlyUsedTools: [], preferredAgents: [], workflowPatterns: [], interactionStyle: 'methodical', feedbackPreference: personaData.communicationPreference === 'brief' ? 'immediate' : 'summary' } as any;
        await repo.update(user!.id, entity);
        try {
          ...
          if (count === 0) await DefaultUserLLMProviderSeed.createDefaultProvidersForUser(dataSource, user!.id, (entity as any).role || 'user');
        } catch (e) { logger.error('Default providers creation failed', e); }
        return { id: entity.id, email: entity.email, firstName: entity.firstName, lastName: entity.lastName, userPersona: entity.userPersona, onboardingProgress: entity.onboardingProgress, behavioralPatterns: entity.behavioralPatterns, updatedAt: entity.updatedAt };
      } catch (error) { set.status = 500; return { error: 'Internal server error' }; }
    })
    ...
    .put('/behavioral-patterns', async ({ set, body, user }) => {
      ...
      if (!validation.success) { set.status = 400; return { error: 'Invalid behavioral patterns data', details: validation.error.errors }; }
      try {
    ...
    .get('/recommendations', async ({ set, user }) => {
      try {
        const repo = DatabaseService.getInstance().getUserRepository();
        const entity = await repo.findById(user!.id);
        if (!entity || !entity.userPersona) { set.status = 400; return { error: 'User persona not found. Please complete onboarding first.' }; }
        const persona: any = entity.userPersona; const behavioral: any = entity.behavioralPatterns;
    ...
    .post('/track-interaction', async ({ set, body, user }) => {
      ...
      if (!validation.success) { set.status = 400; return { error: 'Invalid interaction data', details: validation.error.errors }; }
      try {
    ...
    .get('/optimized-workspace', async ({ set, user }) => {
      try {
        ...
        const entity = await repo.findById(user!.id);
        if (!entity || !entity.userPersona) { set.status = 400; return { error: 'User persona not found. Please complete onboarding first.' }; }
        const workspace = await generateOptimizedWorkspace(entity.userPersona as any, entity.behavioralPatterns as any);
        return workspace;
      } catch (error) { set.status = 500; return { error: 'Internal server error' }; }
    })
  );
...
async function generatePersonaRecommendations(persona: any, behavioralPatterns: any) {
  const recommendations = { recommendedTools: [], recommendedAgents: [], workflowSuggestions: [], uiCustomizations: { layout: persona.workStyle === 'collaborative' ? 'dashboard' : 'focused', density: persona.communicationPreference === 'brief' ? 'compact' : 'comfortable', theme: persona.problemSolvingApproach === 'creative' ? 'creative' : 'default', notifications: persona.communicationPreference === 'brief' ? 'minimal' : 'standard' } } as any;
  return recommendations;
}
