...
async function getAuthUser(authorization?: string | null) {
  ...
  return { id: result.userId!, email: result.email!, role: result.role!, sessionId: result.sessionId };
}

export function registerAuthRoutes(app: any): any {
  return app.group('/api/v1/auth', (app: any) => withOptionalAuth(app)
    // POST /login
    .post('/login', async ({ body, set, request, headers }) => {
      ...
      try {
        ...
        if (user.lockedUntil && new Date(user.lockedUntil) > new Date()) {
          ...
          set.status = 401;
          return { error: 'Authentication Failed', message: 'Account is temporarily locked due to multiple failed login attempts' };
        }
        ...
        if (!isValidPassword) {
          ...
          await userService.updateLoginTracking(user.id, {
            ...
            ...(failedAttempts >= maxAttempts ? { lockedUntil: new Date(Date.now() + lockDuration) } : {})
          });

          await auditService.logSecurityEvent({
            eventType: AuditEventType.LOGIN_FAILED,
            userId: user.id,
            details: { email, reason: 'Invalid password', failedAttempts, accountLocked: failedAttempts >= maxAttempts },
            ipAddress: request.headers.get('x-forwarded-for') || '',
            userAgent: headers['user-agent']
          });
          set.status = 401;
    ...
    .post('/refresh', async ({ body, set }) => {
      ...
      try {
        ...
        const tokens = generateTokens(tokenData.user.id, tokenData.user.email, tokenData.user.role);
        return {
          success: true,
          data: {
            accessToken: tokens.accessToken,
    ...
    .post('/logout', async ({ body, set, headers }) => {
      try {
        ...
        return { success: true, data: { message: 'Logout successful' }, meta: { timestamp: new Date() } };
      } catch (error) {
    ...
    .group('', (g: any) => withRequiredAuth(g)
      .post('/change-password', async ({ body, set, user }) => {
        ...
        logger.error('Change password error', { error, userId: user?.id });
            set.status = 500;
            return { error: 'Internal Server Error', message: 'An error occurred while changing password' };
          }
      })
    )
    ...
    .group('', (g: any) => withRequiredAuth(g)
      .get('/me', async ({ set, user }) => {
        ...
        if (!account) {
            set.status = 404;
            return { success: false, error: { code: 'USER_NOT_FOUND', message: 'User account not found' }, meta: { timestamp: new Date() } };
          }
          return {
            success: true,
            data: {
