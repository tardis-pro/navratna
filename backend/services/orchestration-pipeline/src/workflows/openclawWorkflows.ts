...
export const AMY_MIT_EVENING_REPORT: OpenClawWorkflow = {
  ...
  agentId: 'comms',
  steps: [
    makeStep('generate_report', 'Generate Report', 'python3 /home/pronit/workspace/lobster-scripts/amy-format-report.py'),
    makeStep('send_whatsapp', 'Send WhatsApp Message', 'openclaw message send --channel whatsapp --target "+91XXXXXXXXXX" --message "$(cat /tmp/amy-report.txt)"', ['generate_report']),
  ],
};
...
export const NIDRA_MORNING_DIGEST: OpenClawWorkflow = {
  ...
  agentId: 'nidra',
  steps: [
    makeStep('compile_digest', 'Compile Digest', 'python3 /home/pronit/workspace/lobster-scripts/nidra-compile-digest.py'),
    makeStep('upload_r2_latest', 'Upload R2 Latest', 'bash /home/pronit/workspace/lobster-scripts/r2-upload-wrapper.sh /home/pronit/workspace/nidra-workspace dreams-latest.md /home/pronit/workspace/nidra-workspace/digests/$(date +%Y-%m-%d).md', ['compile_digest']),
    makeStep('upload_r2_dated', 'Upload R2 Dated', 'bash /home/pronit/workspace/lobster-scripts/r2-upload-wrapper.sh /home/pronit/workspace/nidra-workspace digests/$(date +%Y-%m-%d).md /home/pronit/workspace/nidra-workspace/digests/$(date +%Y-%m-%d).md', ['compile_digest']),
    makeStep('update_gist', 'Update Gist', 'python3 /home/pronit/workspace/scripts/gist_update.py --file nidra.md --content-file /home/pronit/workspace/nidra-workspace/digests/$(date +%Y-%m-%d).md', ['compile_digest']),
    makeStep('send_whatsapp', 'Send WhatsApp Message', 'openclaw message send --channel whatsapp --target "+91XXXXXXXXXX" --message "$(cat /tmp/nidra-whatsapp.txt)"', ['compile_digest']),
  ],
};
...
export const KARNA_GIST_DIGEST: OpenClawWorkflow = {
  ...
  agentId: 'growth',
  steps: [
    makeStep('run_digest', 'Run Digest Script', 'python3 /home/pronit/workspace/growth-workspace/scripts/karna_email.py'),
    makeStep('notify', 'Send Notification', 'bash /home/pronit/workspace/lobster-scripts/karna-notify.sh', ['run_digest'], {
      parameters: { command: 'bash /home/pronit/workspace/lobster-scripts/karna-notify.sh', tool: 'bash', stdinFrom: 'run_digest' },
    }),
  ],
};
...
export const BHAGWAN_HEARTBEAT: OpenClawWorkflow = {
  ...
  description: 'Deterministic PB check — zero LLM tokens when idle. Only wakes Bhagwan when real tasks exist.',
  agentId: 'main',
  steps: [
    {
...
export const KARNA_MORNING_HUNT: OpenClawWorkflow = {
  ...
  agentId: 'growth',
  steps: [
    makeStep('read_nidra', 'Read Nidra Reports', 'bash -c \'ls -t /home/pronit/workspace/nidra-workspace/reports/*.md 2>/dev/null | head -3 | xargs cat || echo "No Nidra reports found"\''),
    makeStep('search_leads', 'Search for Leads', 'openclaw chat --agent growth --message "Morning hunt. Read SOUL.md + TOOLS.md. Focus: Gov tenders (GeM/TED) + EU procurement. Search 3-5 qualified leads. APPEND to leads/$(date +%Y-%m-%d).md. Update scrape-log.json. Upload to R2: python3 scripts/r2_upload.py --file leads-latest.md + leads/$(date +%Y-%m-%d).md" --model "zai/glm-5" --no-history', ['read_nidra']),
  ],
};
