  ...
  public async decryptApiKey(
    ...
    return new Promise<string | undefined>((resolve, reject) => {
      ...
      this.eventBusService
        .publish('llm.apikey.decrypt.request', request, {
          metadata: { version: '1.0' },
        })
        .catch((error) => {
          logger.error('Failed to publish API key decryption request', {
            error,
            requestId,
            providerName,
          });
    ...
    this.eventBusService.subscribe(
      ...
      async (message) => {
        try {
          ...
          if (!pendingRequest) {
            logger.warn('Received decryption response for unknown request', {
