...
export class ChatKnowledgeExtractorService {
  ...
  async extractKnowledgeFromConversations(
    ...
    try {
      for (const conversation of conversations) {
        ...
        extractionMethods['fact_extraction'] = (extractionMethods['fact_extraction'] || 0) + facts.length;
        extractionMethods['procedure_extraction'] = (extractionMethods['procedure_extraction'] || 0) + procedures.length;
        ...
        if (options?.extractQA !== false) {
          ...
          extractionMethods['qa_extraction'] = (extractionMethods['qa_extraction'] || 0) + qaResult.length;
        }
        ...
        if (options?.extractDecisions !== false) {
          ...
          extractionMethods['decision_extraction'] = (extractionMethods['decision_extraction'] || 0) + decisions.length;
        }
        ...
        if (options?.extractExpertise !== false) {
          ...
          extractionMethods['expertise_extraction'] = (extractionMethods['expertise_extraction'] || 0) + expertise.length;
        }
        ...
        if (options?.extractLearning !== false) {
          ...
          extractionMethods['learning_extraction'] = (extractionMethods['learning_extraction'] || 0) + learning.length;
        }
      }

      // Apply confidence filter
      if (options?.minConfidence) {
        extractedKnowledge = extractedKnowledge.filter(k => k.confidence >= options.minConfidence!);
      ...
      const totalKnowledgeItems = extractedKnowledge.length + qaPairs.length + decisionPoints.length;
      const avgConfidence = totalKnowledgeItems > 0 
        ? [...extractedKnowledge, ...qaPairs, ...decisionPoints].reduce((sum, item) => sum + item.confidence, 0) / totalKnowledgeItems
        : 0;

      const processingTime = Date.now() - startTime;

      logger.info(`Knowledge extraction completed: ${totalKnowledgeItems} items from ${totalMessagesProcessed} messages in ${processingTime}ms`);
  ...
  async extractFacts(messages: ParsedMessage[], context: ConversationContext): Promise<ExtractedKnowledge[]> {
  ...
  async extractProcedures(messages: ParsedMessage[], context: ConversationContext): Promise<ExtractedKnowledge[]> {
      ...
      for (const pattern of this.procedurePatterns) {
        ...
        while ((match = pattern.exec(content)) !== null) {
          ...
          if (procedureText && procedureText.length > 10) {
            ...
            if (fullMatch.toLowerCase().includes('step') || fullMatch.toLowerCase().includes('first') || 
                fullMatch.toLowerCase().includes('then') || fullMatch.toLowerCase().includes('next')) {
  ...
  async extractQuestionAnswerPairs(messages: ParsedMessage[], context: ConversationContext): Promise<QAPair[]> {
      ...
      if (questionMatch && nextMessage.sender !== currentMessage.sender) {
        ...
        if (confidence > 0.5) {
          qaPairs.push({
            ...
            context,
            confidence,
            tags: this.extractTags(questionMatch + ' ' + nextMessage.content, currentMessage.sender),
            participants: {
  ...
  async extractDecisions(messages: ParsedMessage[], context: ConversationContext): Promise<DecisionPoint[]> {
  ...
  async extractExpertise(conversation: ParsedConversation, context: ConversationContext): Promise<ExpertiseArea[]> {
    const expertiseMap = new Map<string, { domains: Set<string>; evidence: string[]; confidence: number }>();
  ...
  async extractLearningMoments(messages: ParsedMessage[], context: ConversationContext): Promise<LearningMoment[]> {
    ...
    for (let i = 0; i < messages.length - 1; i++) {
      ...
      for (const pattern of this.learningIndicators) {
        ...
        if (match && currentMessage.sender !== nextMessage.sender) {
          ...
          const learningType = this.determineLearningType(currentMessage.content, nextMessage.content);
  ...
  private calculateFactConfidence(match: string, message: ParsedMessage): number {
    ...
    if (match.toLowerCase().includes('is') || match.toLowerCase().includes('are')) confidence += 0.2;
    if (match.length > 50) confidence += 0.1;
    if (message.sender.toLowerCase().includes('assistant') || message.sender.toLowerCase().includes('claude')) confidence += 0.1;
  ...
  private calculateProcedureConfidence(match: string, message: ParsedMessage): number {
    ...
    if (match.toLowerCase().includes('step') || match.toLowerCase().includes('first')) confidence += 0.2;
    if (match.toLowerCase().includes('then') || match.toLowerCase().includes('next')) confidence += 0.1;
  ...
  private calculateDecisionConfidence(match: string, message: ParsedMessage): number {
    ...
    if (match.toLowerCase().includes('decided') || match.toLowerCase().includes('chose')) confidence += 0.2;
    if (match.toLowerCase().includes('because') || match.toLowerCase().includes('reason')) confidence += 0.2;
  ...
  private calculateExpertiseConfidence(match: string, message: ParsedMessage): number {
    ...
    if (match.toLowerCase().includes('experience') || match.toLowerCase().includes('recommend')) confidence += 0.3;
    if (match.length > 40) confidence += 0.1;
    
    return Math.min(confidence, 1.0);
  }

  private calculateLearningConfidence(teacherMsg: ParsedMessage, learnerMsg: ParsedMessage): number {
    let confidence = 0.4;
    
    if (learnerMsg.content.toLowerCase().includes('thanks') || learnerMsg.content.toLowerCase().includes('got it')) confidence += 0.3;
  ...
  private extractTags(content: string, sender: string): string[] {
    ...
    if (content.toLowerCase().includes('step') || content.toLowerCase().includes('process')) tags.push('procedure');
  ...
  private extractTopic(content: string): string {
    ...
    const words = content.split(' ')
      .filter(word => word.length > 3 && !['this', 'that', 'what', 'how', 'why'].includes(word.toLowerCase()))
      .slice(0, 3);
    
    return words.join(' ') || 'general topic';
  }

  private determineLearningType(teacherContent: string, learnerContent: string): LearningMoment['learningType'] {
