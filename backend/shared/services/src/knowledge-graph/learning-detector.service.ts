...
export interface LearningMoment {
  ...
  topic: string;
  content: string;
  type: 'knowledge_transfer' | 'skill_acquisition' | 'concept_clarification' | 'problem_solving' | 'mistake_correction' | 'insight_sharing' | 'experience_sharing';
  learningOutcome: 'understanding' | 'skill_improvement' | 'knowledge_gained' | 'perspective_changed' | 'problem_solved' | 'confusion_resolved';
  timestamp: Date;
  confidence: number;
...
export interface KnowledgeTransfer {
  ...
  transferMethod: 'explanation' | 'demonstration' | 'guidance' | 'correction' | 'questioning' | 'storytelling';
  domain: string;
...
export interface LearningTimelineEntry {
  timestamp: Date;
  event: 'knowledge_gained' | 'skill_practiced' | 'concept_mastered' | 'mistake_made' | 'insight_achieved' | 'knowledge_applied';
  topic: string;
  description: string;
  impact: number; // 0-1
  evidence: string[];
...
export class LearningDetectorService {
  ...
  async detectLearningMoments(
    ...
    try {
      ...
      const questionAnswerMoments = await this.detectQuestionAnswerLearning(participantFilteredMessages);
      ...
      const filteredMoments = learningMoments.filter(moment => moment.confidence >= minConfidence);
  ...
  async identifyKnowledgeTransfer(
    ...
    for (const conversation of conversations) {
      const conversationTransfers = await this.analyzeConversationForTransfer(conversation, options);
  ...
  private async detectQuestionAnswerLearning(messages: ParsedMessage[]): Promise<LearningMoment[]> {
    ...
    for (let i = 0; i < messages.length - 1; i++) {
      ...
      if (this.isQuestion(currentMessage.content)) {
        ...
        for (let j = i + 1; j < Math.min(i + this.MAX_LEARNING_MOMENT_DISTANCE, messages.length); j++) {
          const potentialAnswer = messages[j];
          
          if (potentialAnswer.sender !== currentMessage.sender && 
              this.isAnswer(potentialAnswer.content, currentMessage.content)) {
            
            // Look for learning indicators in subsequent messages
            const learningResponse = this.findLearningResponse(messages, j + 1, currentMessage.sender);
   ...
   */
  private async detectSkillDevelopmentMoments(messages: ParsedMessage[]): Promise<LearningMoment[]> {
    const moments: LearningMoment[] = [];
  ...
  private findLearningResponse(messages: ParsedMessage[], startIndex: number, learner: string): ParsedMessage | null {
  ...
  private findLearningResponses(messages: ParsedMessage[], startIndex: number, excludeSender: string): ParsedMessage[] {
  ...
  private findInsightContext(messages: ParsedMessage[], insightMessage: ParsedMessage): {teacher: string, context: string} | null {
  ...
  private async buildMetadata(params: any, confidence: number, effectiveness: number): Promise<LearningMoment['metadata']> {
    return {
  ...
  private extractTimestamp(params: any): Date {
    return params.question?.timestamp || 
  ...
  private extractTopicFromContent(content: string): string {
    ...
    const commonWords = new Set(['this', 'that', 'with', 'from', 'they', 'them', 'their', 'have', 'been', 'were', 'said', 'each', 'more', 'some', 'like', 'into', 'time', 'very', 'then', 'than', 'only', 'come', 'over', 'just', 'also']);
  ...
  private extractAcknowledgments(params: any): string[] {
    ...
    const responseContent = params.learningResponse?.content || params.acceptanceResponse?.content || '';
  ...
  private findRetentionIndicators(params: any): string[] {
    ...
    if (this.indicatesLearning(params.learningResponse?.content || '')) indicators.push('explicit_acknowledgment');
  ...
  private async analyzeConversationForTransfer(conversation: ParsedConversation, options: LearningDetectionOptions): Promise<KnowledgeTransfer[]> {
  ...
  private buildLearningTimeline(moments: LearningMoment[]): LearningTimelineEntry[] {
    return moments.map(moment => ({
      ...
      teacher: moment.teacher !== 'self' && moment.teacher !== 'practice' ? moment.teacher : undefined,
      context: moment.context.domain
    }));
  }

  private mapLearningOutcomeToEvent(outcome: LearningMoment['learningOutcome']): LearningTimelineEntry['event'] {
  ...
  private async analyzeSkillProgressions(moments: LearningMoment[], participant: string): Promise<SkillProgression[]> {
  ...
  private calculateKnowledgeGrowthMetrics(moments: LearningMoment[]): KnowledgeGrowthMetrics {
    ...
    return {
      ...
      applicationRate: 0.6,
      transferRate: 0.5,
      curiosityIndex: moments.filter(m => m.type === 'knowledge_transfer').length / moments.length,
      masteryIndicators: []
    };
  }

  private async identifyLearningPattern(moments: LearningMoment[], participant: string): Promise<LearningProgression['learningPattern']> {
  ...
  private async assessMotivation(moments: LearningMoment[], participant: string): Promise<LearningProgression['motivation']> {
  ...
  private calculateLearningEffectiveness(moments: LearningMoment[]): LearningProgression['effectiveness'] {
  ...
  private calculateOverallMetrics(moments: LearningMoment[], progressions: LearningProgression[], transfers: KnowledgeTransfer[]): LearningInsights['overallMetrics'] {
    ...
    return {
      ...
      knowledgeFlowStrength: transfers.reduce((sum, t) => sum + t.effectiveness, 0) / Math.max(transfers.length, 1),
      learningVelocity: moments.length / Math.max(1, this.calculateTimeSpan(moments))
    };
  }

  private async identifyLearningPatterns(moments: LearningMoment[], progressions: LearningProgression[], transfers: KnowledgeTransfer[]): Promise<LearningInsights['patterns']> {
  ...
  private async generateRecommendations(moments: LearningMoment[], progressions: LearningProgression[], transfers: KnowledgeTransfer[]): Promise<LearningInsights['recommendations']> {
  ...
  private analyzeLearningTrends(moments: LearningMoment[], progressions: LearningProgression[]): LearningInsights['trends'] {
  ...
  private async performNetworkAnalysis(moments: LearningMoment[], transfers: KnowledgeTransfer[]): Promise<LearningInsights['networkAnalysis']> {
  ...
  // Utility methods
  private filterMessagesByTimeWindow(messages: ParsedMessage[], timeWindow?: LearningDetectionOptions['timeWindow']): ParsedMessage[] {
  ...
  private async enhanceLearningMoments(moments: LearningMoment[], allMessages: ParsedMessage[]): Promise<LearningMoment[]> {
