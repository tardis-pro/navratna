...
export class RelationshipDetector {
  ...
  ): Promise<Omit<KnowledgeRelationship, 'id' | 'createdAt'>[]> {
  ...
  private detectSpecificRelationship(
    newItem: KnowledgeItem,
    existingItem: KnowledgeItem
  ): string | null {
    ...
    if (
      this.containsReference(newContent, existingContent) ||
      this.containsReference(existingContent, newContent)
    ) {
      return 'references';
    }
  ...
  private isFollowUp(newItem: KnowledgeItem, existingItem: KnowledgeItem): boolean {
    ...
    return (
      followUpPatterns.some((pattern) => newContent.match(pattern)) &&
  ...
  private isExample(newItem: KnowledgeItem, existingItem: KnowledgeItem): boolean {
    ...
    return (
      examplePatterns.some((pattern) => newContent.match(pattern)) ||
  ...
  private isImplementation(newItem: KnowledgeItem, existingItem: KnowledgeItem): boolean {
    ...
    return (
      (newItem.type === 'PROCEDURAL' && existingItem.type === 'CONCEPTUAL') ||
  ...
  private isUpdate(newItem: KnowledgeItem, existingItem: KnowledgeItem): boolean {
    ...
    return (
      newItem.sourceIdentifier === existingItem.sourceIdentifier &&
  ...
  private detectContradiction(newItem: KnowledgeItem, existingItem: KnowledgeItem): boolean {
    ...
    return contradictionPatterns.some(
      (pattern) => newContent.match(pattern) || existingContent.match(pattern)
    );
  }

  private async detectTagBasedRelationships(
    newItem: KnowledgeItem
  ): Promise<Omit<KnowledgeRelationship, 'id' | 'createdAt'>[]> {
    const relationships: Omit<KnowledgeRelationship, 'id' | 'createdAt'>[] = [];
    ...
    try {
      ...
      for (const relatedItem of relatedItems) {
        ...
        const tagOverlapRatio =
          commonTags.length / Math.max(newItem.tags.length, relatedItem.tags.length);

        if (tagOverlapRatio > 0.3) {
          // At least 30% tag overlap
          relationships.push({
            sourceItemId: newItem.id,
