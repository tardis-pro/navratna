...
export class FixSecurityLevelEnum1704007000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TYPE "tool_definitions_security_level_enum_new" RENAME TO "tool_definitions_security_level_enum"`
    );
    await queryRunner.query(
      `ALTER TABLE "tool_definitions" RENAME COLUMN "security_level_new" TO "security_level"`
    );

    // Step 5: Make the column NOT NULL (it should be required)
    await queryRunner.query(
      `ALTER TABLE "tool_definitions" ALTER COLUMN "security_level" SET NOT NULL`
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`DROP TYPE "tool_definitions_security_level_enum"`);
    await queryRunner.query(
      `ALTER TYPE "tool_definitions_security_level_enum_old" RENAME TO "tool_definitions_security_level_enum"`
    );
    await queryRunner.query(
      `ALTER TABLE "tool_definitions" RENAME COLUMN "security_level_old" TO "security_level"`
    );
    await queryRunner.query(
      `ALTER TABLE "tool_definitions" ALTER COLUMN "security_level" SET NOT NULL`
    );
