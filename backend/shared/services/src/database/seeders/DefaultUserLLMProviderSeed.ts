...
export class DefaultUserLLMProviderSeed {
  ...
  private static getDefaultProvidersForRole(role: string): Array<{
    ...
    baseProviders.push({
      ...
      description:
        'Anthropic Claude models with vision and tool calling - Add your API key in settings',
    ...
    baseProviders.push({
      ...
      description:
        'Local LM Studio instance for running open-source models privately - DEFAULT PROVIDER',
  ...
  static async cleanupInactiveProviders(dataSource: DataSource, userId: string): Promise<void> {
    try {
      ...
      const providersToDelete = inactiveProviders.filter(
        (provider) => !provider.lastUsedAt || provider.lastUsedAt < thirtyDaysAgo
      );
