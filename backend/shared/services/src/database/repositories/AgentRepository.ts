...
export class AgentRepository extends BaseRepository<Agent> {
  ...
  public async getActiveAgentById(agentId: string): Promise<Agent | null> {
    try {
      ...
      logger.error('Error getting active agent by ID', {
        agentId,
  ...
  public async createAgent(agentData: {
    ...
    try {
      ...
      if (agentData.persona && !agentData.legacyPersona) {
        ...
        logger.warn('Using deprecated persona field as legacyPersona', {
  ...
  public async updateAgent(
    ...
    try {
      ...
      if (updateData.legacyPersona !== undefined)
        updatePayload.legacyPersona = updateData.legacyPersona;
      ...
      if (updateData.intelligenceConfig !== undefined)
        updatePayload.intelligenceConfig = updateData.intelligenceConfig;
      if (updateData.securityContext !== undefined)
        updatePayload.securityContext = updateData.securityContext;
      ...
      if (updateData.capabilities !== undefined)
        updatePayload.capabilities = updateData.capabilities;
      ...
      if (updateData.systemPrompt !== undefined)
        updatePayload.systemPrompt = updateData.systemPrompt;
  ...
  public async getAgentCapabilitiesConfig(agentId: string): Promise<{
    ...
    try {
      ...
      logger.error('Error getting agent capabilities config', {
        agentId,
        error: (error as Error).message,
      });
      throw error;
  ...
  public async hasAgentsUsingProvider(apiType: string): Promise<boolean> {
    try {
      ...
      logger.error('Error checking agents using provider', {
        apiType,
        error: (error as Error).message,
      });
      throw error;
  ...
  public async getAgentsUsingProvider(apiType: string): Promise<Agent[]> {
    try {
      ...
      logger.error('Error getting agents using provider', {
        apiType,
        error: (error as Error).message,
      });
      throw error;
