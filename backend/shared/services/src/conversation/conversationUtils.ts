...
export class ConversationUtils {
  ...
  static analyzeConversationFlow(
    ...
    if (diversityScore < 0.4) {
      suggestions.push(
        'Low speaker diversity detected. Consider encouraging more participants to join the conversation.'
      );
    }

    if (backToBackCount > 3) {
      suggestions.push(
        'Too many consecutive messages from the same speaker. Encourage speaker transitions for better flow.'
      );
    }

    if (dominationScore > 0.7) {
      suggestions.push(
        'One participant is dominating the conversation. Balance participation across all speakers.'
      );
    }

    if (avgContributionScore < 1.5) {
      suggestions.push(
        'Low engagement detected. Consider introducing new topics or asking open-ended questions.'
      );
    }

    if (balanceScore < 0.3) {
      suggestions.push(
        'Unbalanced participation. Encourage quieter participants to share their perspectives.'
      );
    }
  ...
  static getConversationInsights(
    ...
    for (let i = 1; i < messageHistory.length; i++) {
      const timeDiff =
        (new Date(messageHistory[i].timestamp).getTime() -
          new Date(messageHistory[i - 1].timestamp).getTime()) /
        1000;
    ...
    if (topContributors.length > 0) {
      ...
      if (topPercentage > 60) {
        ...
        topPercentage > 40 ||
        (topContributors.length > 1 && topContributors[1].percentage < 10)
      ) {
