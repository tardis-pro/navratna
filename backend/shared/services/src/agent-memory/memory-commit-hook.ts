...
import { ActionRecommendation, ToolDefinition, SecurityLevel, TemporaryLearning } from '@uaip/types';
...
export class MemoryCommitHook {
  ...
  async commitActionExecution(
    ...
    try {
      ...
      logger.debug(`Memory committed for agent ${agentId}: ${entry.type} (${success ? 'success' : 'failure'})`);
  ...
  async commitToolUsage(
    ...
    try {
      ...
      logger.debug(`Tool usage memory committed for agent ${agentId}: ${tool.name} (${success ? 'success' : 'failure'})`);
  ...
  private calculateToolSignificance(tool: ToolDefinition, success: boolean, duration?: number) {
    ...
    if (tool.securityLevel === SecurityLevel.HIGH || tool.securityLevel === SecurityLevel.CRITICAL) {
      importance += 0.3;
    }
    
    // Higher importance for longer executions
    if (duration && duration > 60000) { // > 1 minute
      importance += 0.2;
    }
  ...
  private generateTags(entry: MemoryEntry): string[] {
    ...
    if (entry.content.tool) {
      tags.push(`category_${entry.content.tool.category}`, `security_${entry.content.tool.securityLevel}`);
    }
