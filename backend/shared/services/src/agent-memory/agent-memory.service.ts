...
export class AgentMemoryService {
  ...
  async updateEmotionalState(agentId: number, emotion: Partial<any>): Promise<void> {
    await this.workingMemoryManager.updateEmotionalState(agentId, emotion);
  }

  // Episodic Memory Operations
  async getEpisodicMemory(agentId: number, query: EpisodicQuery): Promise<Episode[]> {
  ...
  async getSemanticMemory(agentId: number, concept: string): Promise<SemanticMemory[]> {
    const conceptMemory = await this.semanticMemoryManager.getConcept(agentId, concept);
    return conceptMemory ? [conceptMemory] : [];
  }

  async updateSemanticMemory(agentId: number, concept: SemanticMemory): Promise<void> {
  ...
  async searchMemories(agentId: number, query: string, memoryTypes: ('working' | 'episodic' | 'semantic')[] = ['episodic', 'semantic']): Promise<{
    episodes: Episode[];
  ...
  // Memory Cleanup
  async cleanupMemory(agentId: number, options: {
  ...
  async exportAgentMemory(agentId: number): Promise<{
